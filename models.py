...
def linear_svm(train_x, train_y, test_x, test_y):
    ...
    # bagging = BaggingClassifier(clf, max_samples=0.5, max_features=0.5, n_estimators=10)
    clf.fit(train_x, train_y)
    print("Train time: %s" % (time.time() - start))
    dump(clf, 'models/linearSVM.model')
    judge(clf, test_x, test_y)
    plot_roc(clf, test_x, test_y)
...
def logic_reg(train_x, train_y, test_x, test_y):
    ...
    clf.fit(train_x, train_y)
    print("Train time: %s" % (time.time() - start))
    dump(clf, 'models/logic_reg.model')
    judge(clf, test_x, test_y)
    plot_roc(clf, test_x, test_y)
...
def naive_bay(train_x, train_y, test_x, test_y):
    ...
    clf.fit(train_x, train_y)
    print("Train time: %s" % (time.time() - start))
    dump(clf, 'models/GaussianNB.model')
    judge(clf, test_x, test_y)
...
def decision_tree(train_x, train_y, test_x, test_y):
    ...
    clf.fit(train_x, train_y)
    print("Train time: %s" % (time.time() - start))
    dump(clf, 'models/decision_tree.model')
    judge(clf, test_x, test_y)
